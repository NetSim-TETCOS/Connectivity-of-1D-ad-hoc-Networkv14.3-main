...
fprintf(fileid,'X1,X2,Range,Packets Received');
fclose(fileid);

rng('default')
ran_arr = 1000*rand(100,2);
...
for j = 50:50:1000
    ...
    for i = 1:100
        ...
        cmd="ConfigWriter.exe " + string(ran_arr(i,1)) + " "  + string(ran_arr(i,2)) + " " +  string(j);    
        disp(cmd)
        system(cmd);
        ...
        if(isfile('IOPath\Metrics.xml'))
            ...
            if(fileid)
                fprintf(fileid,'\n%f,%f,%f,',ran_arr(i,1),ran_arr(i,2),j);
                fclose(fileid);
            end
            ...
            fileid=fopen('result.csv','a');
            if(fileid)
                fprintf(fileid,'\n%f,%f,%f,crash',ran_arr(i,1),ran_arr(i,2),j);
                fclose(fileid);
            end
        end
