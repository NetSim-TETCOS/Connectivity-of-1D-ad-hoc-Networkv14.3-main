...
with open("result.csv", "w") as csvfile:
    csvfile.write("X1,X2,X3,X4,X5,Range,Packets Received,")

# Create random UE positions
np.random.seed(0)
ran_arr = np.round(1000 * np.random.rand(100, 5), 2)
...
for j in range(50, 1001, 50):
    for i in range(100):  # Fixed indentation issue here
        ...
        cmd = f'ConfigWriter.exe {ran_arr[i,0]} {ran_arr[i,1]} {ran_arr[i,2]} {ran_arr[i,3]} {ran_arr[i,4]} {j}'
        print(cmd)
        os.system(cmd)
        ...
        if os.path.isfile("IOPath\\Metrics.xml"):
            ...
            with open("result.csv", "a") as csvfile:
                csvfile.write(f"\n{ran_arr[i,0]},{ran_arr[i,1]},{ran_arr[i,2]},{ran_arr[i,3]},{ran_arr[i,4]},{j},")
