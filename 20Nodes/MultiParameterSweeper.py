...
with open("result.csv", "w") as csvfile:
    csvfile.write("X1,X2,X3,X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,X17,X18,X19,X20,Range,Packets Received,")

# Create random UE positions
np.random.seed(0)
ran_arr = np.round(1000 * np.random.rand(100, 20), 2)
...
for j in range(50, 1001, 50):
    for i in range(100):  # Fixed indentation issue here
        ...
        cmd = f'ConfigWriter.exe {ran_arr[i,0]} {ran_arr[i,1]} {ran_arr[i,2]} {ran_arr[i,3]} {ran_arr[i,4]} {ran_arr[i,5]} {ran_arr[i,6]} {ran_arr[i,7]} {ran_arr[i,8]} {ran_arr[i,9]} {ran_arr[i,10]} {ran_arr[i,11]} {ran_arr[i,12]} {ran_arr[i,13]} {ran_arr[i,14]} {ran_arr[i,15]} {ran_arr[i,16]} {ran_arr[i,17]} {ran_arr[i,18]} {ran_arr[i,19]} {j}'
        print(cmd)
        os.system(cmd)
        ...
        if os.path.isfile("IOPath\\Metrics.xml"):
            ...
            with open("result.csv", "a") as csvfile:
                csvfile.write(f"\n{ran_arr[i,0]},{ran_arr[i,1]},{ran_arr[i,2]},{ran_arr[i,3]},{ran_arr[i,4]},{ran_arr[i,5]},{ran_arr[i,6]},{ran_arr[i,7]},{ran_arr[i,8]},{ran_arr[i,9]},{ran_arr[i,10]},{ran_arr[i,11]},{ran_arr[i,12]},{ran_arr[i,13]},{ran_arr[i,14]},{ran_arr[i,15]},{ran_arr[i,16]},{ran_arr[i,17]},{ran_arr[i,18]},{ran_arr[i,19]},{j},")
            ...
            with open("result.csv", "a") as csvfile:
                csvfile.write(f"\n{i},crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,crash,")
